...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    fun testAllRealPing() {
        ...
        val mainJob = runOnDefaultDispatcher {
            ...
            // Filter profiles that can be TCP pinged and prepare addresses
            val validProfiles = mutableListOf<ProxyEntity>()
            ...
            for (profile in profilesUnfiltered) {
                ...
                if (!address.isIpAddress()) {
                    try {
                        ...
                        profile.status = 2
                        profile.error = ErrorHandler.handleNetworkError(e, "domain resolution")
                        test.update(profile)
                        continue
                    }
                }
                ...
                addresses.add("$address:${profile.requireBean().serverPort}")
                profile.status = 0
                test.insert(profile)
            }
            ...
            try {
                ...
                val addressesJson = JSONArray(addresses).toString()
                
                // Call native ParallelPing function with timeout from settings
                val timeout = SettingsManager.Network.getPingTimeoutMs()
                val resultsJson = Libcore.parallelPing(addressesJson, timeout)
                
                // Parse results
                val resultsArray = JSONArray(resultsJson)
                
                // Update profiles with results
                for (i in 0 until resultsArray.length()) {
                    if (i >= validProfiles.size) break
                    
                    val result = resultsArray.getJSONObject(i)
                    val profile = validProfiles[i]
                    
                    if (result.getBoolean("success")) {
                        profile.status = 1
                        profile.ping = result.getInt("ping_ms")
                    } else {
                        profile.status = 2
                        val errorMsg = result.optString("error", "Connection failed")
                        profile.error = ErrorHandler.handleNetworkError(Exception(errorMsg), "TCP ping")
                    }
                    test.update(profile)
                }
                
            } catch (e: Exception) {
                Logs.w("ParallelPing error", e)
                // Fallback: mark all as failed with proper error handling
                for (profile in validProfiles) {
                    if (profile.status == 0) {
                        profile.status = 2
                        profile.error = ErrorHandler.handleNetworkError(e, "parallel ping test")
                        test.update(profile)
                    }
                }
            }
    ...
    fun refreshAll() {
        ...
        val mainJob = runOnDefaultDispatcher {
            ...
            try {
                ...
                Logs.w("ParallelPing error during refresh", e)
                // Fallback: mark all as failed with proper error handling
                for (profile in validProfiles) {
                    if (profile.status == 0) {
                        profile.status = 2
            ...
            // Step 2: Remove duplicates (based on server address + port + method)
            val seenConfigs = mutableSetOf<String>()
            val profilesToRemove = mutableListOf<ProxyEntity>()
            
            for (profile in profilesUnfiltered) {
                val bean = profile.requireBean()
                val configKey = "${bean.serverAddress}:${bean.serverPort}:${bean.javaClass.simpleName}"
            ...
            for (profile in profilesToRemove) {
                try {
                    ...
                    Logs.w("Failed to remove duplicate profile", e)
                }
            }

            // Step 3: Sort by ping (available configs first, then by ping time)
            val availableProfiles = profilesUnfiltered.filter { it.status == 1 }.sortedBy { it.ping }
            val unavailableProfiles = profilesUnfiltered.filter { it.status != 1 }
            
            // Update profile order in database
            var order = 0
            for (profile in availableProfiles + unavailableProfiles) {
                if (!profilesToRemove.contains(profile)) {
                    profile.userOrder = order++
                    try {
                        ProfileManager.updateProfile(profile)
                    } catch (e: Exception) {
                        Logs.w("Failed to update profile order", e)
                    }
                }
            }
            ...
            onMainDispatcher {
                ...
                // Step 4: Scroll to top if enabled in settings
                if (SettingsManager.UI.shouldAutoScrollToTopAfterRefresh()) {
                    (binding.recyclerView.layoutManager as? LinearLayoutManager)?.scrollToPositionWithOffset(0, 0)
                }
                
                // Step 5: Show result in Snackbar
                val availableCount = availableProfiles.size
                val totalCount = profilesUnfiltered.size - duplicatesRemoved
                
                val message = if (duplicatesRemoved > 0) {
                    getString(R.string.refresh_complete_with_duplicates, totalCount, availableCount, duplicatesRemoved)
                } else {
                    getString(R.string.refresh_complete, totalCount, availableCount)
                }
                ...
                // Check for persistently failed configs and offer to remove them
                checkAndRemoveFailedConfigs(profilesUnfiltered.filter { !profilesToRemove.contains(it) })
            }
        }
    ...
    /**
     * Check for configs that have failed multiple times and ask user to remove them
     */
    private fun checkAndRemoveFailedConfigs(profiles: List<ProxyEntity>) {
        val maxFailures = SettingsManager.UI.getRemoveFailedConfigsAfterAttempts()
        if (maxFailures <= 0) return // Feature disabled
        
        // Find configs that have failed multiple times
        // We'll use a simple heuristic: configs with status=2 and specific error patterns
        val failedConfigs = profiles.filter { profile ->
            profile.status == 2 && profile.error?.let { error ->
                // Check for persistent connection errors (not temporary network issues)
                error.contains("Connection refused") || 
                error.contains("Host unreachable") ||
                error.contains("Connection failed") ||
                error.contains("timeout") ||
                error.contains("DNS resolution failed")
            } == true
        }
        
        if (failedConfigs.isEmpty()) return
        
        // Group by error type to avoid removing configs with temporary issues
        val persistentlyFailedConfigs = failedConfigs.filter { profile ->
            profile.error?.let { error ->
                error.contains("Connection refused") || 
                error.contains("Host unreachable")
            } == true
        }
        
        if (persistentlyFailedConfigs.isEmpty()) return
        
        // Ask user for confirmation
        val message = if (persistentlyFailedConfigs.size == 1) {
            "Remove 1 config that appears to be permanently unavailable?"
        } else {
            "Remove ${persistentlyFailedConfigs.size} configs that appear to be permanently unavailable?"
        }
        
        MaterialAlertDialogBuilder(requireContext())
            .setTitle("Remove Failed Configs")
            .setMessage(message)
            .setPositiveButton("Remove") { _, _ ->
                runOnDefaultDispatcher {
                    var removedCount = 0
                    for (profile in persistentlyFailedConfigs) {
                        try {
                            ProfileManager.deleteProfile(profile.id)
                            removedCount++
                        } catch (e: Exception) {
                            Logs.w("Failed to remove failed config", e)
                        }
                    }
                    
                    onMainDispatcher {
                        if (removedCount > 0) {
                            snackbar("Removed $removedCount failed configs").show()
                            GroupManager.postReload(DataStore.currentGroupId())
                        }
                    }
                }
