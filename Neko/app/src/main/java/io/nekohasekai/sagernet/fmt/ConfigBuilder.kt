...
import io.nekohasekai.sagernet.doh.IntraDohResolverImpl
import io.nekohasekai.sagernet.doh.IPMapImpl
import io.nekohasekai.sagernet.doh.DnsPadding
import io.nekohasekai.sagernet.doh.IntraDohResolver
import moe.matsuri.nb4a.SingBoxOptions.IntraDohServerOptions
...
fun buildConfig(
    ...
    return MyOptions().apply {
        ...
        dns = DNSOptions().apply {
            ...
            // Use selected DoH server as primary remote DNS
            if (DataStore.dohServers.isNotEmpty()) {
                servers.add(DNSServerOptions().apply {
                    tag = "doh-selected"
                    address = DataStore.dohServers
                    transport = "https"
                })
            }
        }
        ...
        fun autoDnsDomainStrategy(s: String): String {
            ...
            return when (ipv6Mode) {
                ...
                else -> "prefer_ipv4" // CHANGED FROM null TO DEFAULT VALUE
            }
        }
...
//                        val needKeepAliveInterval = keepAliveInterval !in intArrayOf(0, 15)
        ...
        remoteDns.firstOrNull().let {
            ...
            if (!forTest) dns.servers.add(DNSServerOptions().apply {
                ...
                strategy = autoDnsDomainStrategy(SingBoxOptionsUtil.domainStrategy(tag))
                transport = "https" // ADDED BY MANUS
            })
        }
ort = "https" // ADDED BY MANUS
            })
        }
       }
        ...
        if (forTest) {
            ...
            // Iranian apps direct routing
            if (SettingsManager.Routing.isIranianAppsDirectEnabled()) {
                val iranianPackages = SettingsManager.Routing.getIranianAppPackages()
                if (iranianPackages.isNotEmpty()) {
                    PackageCache.awaitLoadSync()
                    val iranianUids = mutableListOf<Int>()
                    
                    for (packageName in iranianPackages) {
                        val uid = PackageCache[packageName]
                        if (uid != null && uid > 0) {
                            iranianUids.add(uid)
                        }
                    }
                    
                    if (iranianUids.isNotEmpty()) {
                        // Add rule at the beginning to ensure it has high priority
                        route.rules.add(0, Rule_DefaultOptions().apply {
                            user_id = iranianUids
                            outbound = TAG_DIRECT
                        })
