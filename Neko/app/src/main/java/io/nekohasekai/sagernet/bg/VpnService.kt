...
import io.nekohasekai.sagernet.doh.IntraListener
import io.nekohasekai.sagernet.doh.IntraTunnel
import io.nekohasekai.sagernet.doh.Summary
...
class VpnService : BaseVpnService(),
    ...
    private var intraTunnel: IntraTunnel? = null
...
//        val tunOptions = JSONObject(tunOptionsJson)
        ...
        // Initialize and start IntraTunnel
        val dohUrl = DataStore.dohServers.split("\n").firstOrNull()?.trim() ?: ""
        if (dohUrl.isNotEmpty()) {
            val intraDohResolver = IntraDohResolverImpl(dohUrl)
            intraTunnel = IntraTunnel(
                FAKEDNS_VLAN4_CLIENT, // fakedns
                intraDohResolver,
                conn!!.fd, // tunFd
                { socket -> protect(socket) }, // protector
                object : IntraListener {
                    override fun onQuery(url: String) {
                        Logs.d("Intra DoH Query: $url")
                    }

                    override fun onResponse(summary: Summary) {
                        Logs.d("Intra DoH Response: ${summary.latency}ms, Status: ${summary.status}")
                    }
                }
            )
            intraTunnel?.start()
