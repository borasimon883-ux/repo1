...
class ProxyInstance(profile: ProxyEntity, var service: BaseService.Interface? = null) :
    ...
    override suspend fun loadConfig() {
        try {
            ...
            Logs.d("DNS transport registered successfully")
        } catch (e: Exception) {
            Logs.w("Failed to register DNS transport: ${ErrorHandler.handleDnsError(e, "local resolver")}")
            // Continue without local DNS transport - fallback to system resolver
        }
        super.loadConfig()
    }
    ...
    override fun close() {
        try {
            ...
            Logs.d("DNS transport unregistered successfully")
        } catch (e: Exception) {
            Logs.w("Failed to unregister DNS transport: ${ErrorHandler.handleDnsError(e, "local resolver cleanup")}")
